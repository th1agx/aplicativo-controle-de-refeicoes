...
export default function RegRefeicoesManha({ navigation, route }) {
  ...
  useEffect(() => {
    const loadTurmas = () => {
      const creche = turmasNome.filter(
        (turma) => turma.ano_Escolar === '03 anos' && turma.turno_Descr === 'MANHÃ'
      );
      const preEscola = turmasNome.filter(
        (turma) => (turma.ano_Escolar === '04 anos' || turma.ano_Escolar === '05 anos') && turma.turno_Descr === 'MANHÃ'
      );
    ...
    const loadRefeicoes = async () => {
      try {
        const storedData = await AsyncStorage.getItem('refeicoesPorDataManha');
  ...
  const calcularTotais = (turmas) => {
    return turmas.reduce((totais, turma) => {
      totais.cafe += parseInt(refeicoes[turma.codTurma]?.cafe || 0);
      totais.rept += parseInt(refeicoes[turma.codTurma]?.rept || 0);
      totais.almoco += parseInt(refeicoes[turma.codTurma]?.almoco || 0);
  ...
  const handleSaveManha = async () => {
    try {
      ...
      dataObject[formattedSelectedDate] = dadosManha;

      await AsyncStorage.setItem('refeicoesPorDataManha', JSON.stringify(dataObject));

      setMensagemSucesso('Registros da manhã salvos com sucesso!');
      setIsSaved(true);
      
      setTimeout(() => setMensagemSucesso(''), 5000);
    } catch (error) {
      console.error('Erro ao salvar os dados da manhã: ', error);
    }
  };
  ...
  return (
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.turno}>MANHÃ</Text>
        <Text style={styles.nomedata}>DATA: {formattedSelectedDate}</Text>
      </View>

      <View>
        <TouchableOpacity style={styles.salvar} onPress={handleSaveManha}>
      ...
      {/* CRECHE - 0 A 3 ANOS */}
        ...
        {crecheTurmas.map((turma) => (
          <View key={turma.codTurma} style={styles.turmaRow}>
            ...
            {['cafe', 'rept', 'almoco', 'rept1', 'fruta', 'rept2'].map((item) => (
              <TextInput
                key={item}
